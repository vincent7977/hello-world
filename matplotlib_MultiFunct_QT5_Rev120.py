...
for particular data set""")
    ...
    def fileOpenF9Borg_Multi(self):
        ...
        self.press_time = False        # Set initial flag to false
 ...
 # Initialize Main Plot Window :           
    def init_plot(self):
        ...
        plotwidget.button15 = QtWidgets.QPushButton('Plot-Pressure-Time History, SPL calc')
        plotwidget.button15.clicked.connect(self.press_time_button_click)
...
# set the layout
 ...
 #       layout2.addWidget(plotwidget.button4)
        ...
        layout2.addWidget(plotwidget.button15)   # PLot psi-time (SPL-calc) button
...
# Define Liftoff Times :  Days, Hrs, Mins, sec & Total Time
    ...
    def plot_AccelTime(self):
        ...
        if self.filetype == 'F9_multi':
           #  Add some stuff
            ...
            for j in [0]:
                ...
                if self.press_time is True:       # test for psi-time plot
                    ax.set_title(f' SPL, Pressure-Time History, Channel/Axis = {self.save_channel}')
                else:
        ...
        if self.filetype == 'F9_matlab':
            ...
            for j in [0]:
                ...
                if self.psi_time is True:       # test for psi-time plot
                    ax.set_title(f' SPL, Pressure-Time History, Channel/Axis = {self.save_channel}')
                else:
                    ax.set_title(f'Accel-Time History, Channel/Axis = {self.save_channel}')
    ...
    def press_time_button_click(self):
        press_time = 'spl_clicked'
        self.press_time = press_time
        print('Psi - Time plot function has been selected')
        print('boolen for psi-time button = ', press_time) 
        self.plot_AccelTime()
...
class AccelTime_Window(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
       ...
       # start Accel-Time & PSD  plot  process
        ...
        self.octn = "True"
        self.ftime = "True"
        self.windendstate = "False"
        sw.psi_time = True
        psi_time = sw.psi_time
        self.psi_time = psi_time
        print('psi time is', psi_time)
        ...
        if self.psi_time == 'spl_clicked':
            self.SpinBox9.setProperty("value", 3)  # Set init value Oct = 3 for 1/3 oct SPL calc
        else:
            self.SpinBox9.setProperty("value", 24) # Set all others to Oct = 24   
        ...
        self.checkBox9 = QCheckBox("Select WindowEnds")
        self.checkBox9.stateChanged.connect(self.btnstate11)
        self.checkBox9.setChecked(False)
        self.main_layout.addWidget(self.checkBox9)
...
# Define Check Boxes from Accel_Time Function:
   ...
   # Start Calculations, Accel-Time History
       # Define selection range of Data
        ...
        if ftime is True:                  # Check to apply Fixtime to Data
        ...
        if windends:
            d2 = dsp.windowends(value_y, portion=0.01, ends=windends)
            value_y = d2
            self.value_y = d2
    ...
    def btnstate11(self,checkBox9):
        if self.checkBox9.isChecked() == True:
            print("WindowEnds is selected")
            windendstate = "True"
            self.windendstate = windendstate
        else:
            print("WindowEnds is not selected")
            self.windendstate = "False"
    ...
    def comboState10(self):
        ...
        print(self.windendsComboBox.currentText())
...
class PsdPlot_Window(AccelTime_Window):
    ...
    def __init__(self, parent=None):
       ...
       #  Initialize options
        ...
        self.setWindowTitle("PSD calculation - Modified Function")
        self.l5 = QLabel()
...
class SplPlot_Window(AccelTime_Window):

    def __init__(self, parent=None):
        super().__init__(parent)
        self.sw1 = self.parent
        sw1 = self.sw1
        RP = {  0  : ('2.0e-5 Pascals', 2.0e-5), 1 : ('2.9e-9 PSI', 2.9e-9),}   # Define ref press. dictionary
        self.RP = RP
        RPtext = (  RP[0][0], RP[1][0],)
        self.RPtext = RPtext
        RPtext_current = {}
       ...
       #  Initialize options
        ...
        self.setWindowTitle("Sound Pressure Level (SPL) calculation ")
        self.l5 = QLabel()
        self.l5.setText("              ")    # Add blank space
        self.main_layout.addWidget(self.l5)

        self.l1 = QLabel()
        self.l1.setText(" SPL Plot Parameters :         ")    # Code block title
        self.main_layout.addWidget(self.l1)

        self.pushButton = QtWidgets.QPushButton("Plot")
        self.main_layout.addWidget(self.pushButton) 
        self.pushButton.clicked.connect(self.on_Spl_PlotpushButton_clicked)
#        self.l1 =  QLabel()
        ...
        self.l2.setText("npseg - Bandwidth(Hz)")
        self.main_layout.addWidget(self.l2)
        self.doubleSpinBox20 = QDoubleSpinBox(self.main_widget)
        self.doubleSpinBox20.setProperty("value", 5.0)
        self.doubleSpinBox20.setSuffix(" Hz")
        self.dsbval20 = self.doubleSpinBox20.value()
        self.doubleSpinBox20.valueChanged.connect(self.spinState20)
        self.main_layout.addWidget(self.doubleSpinBox20)

        self.l3 = QLabel()
        self.l3.setText("Timeslice")
        self.main_layout.addWidget(self.l3)
        self.doubleSpinBox21 = QDoubleSpinBox(self.main_widget)
        self.doubleSpinBox21.setProperty("value", 1.0)
        self.doubleSpinBox21.setRange(0, 10000)
        self.doubleSpinBox21.setSingleStep(0.01)
        self.doubleSpinBox21.valueChanged.connect(self.spinState21)
        self.main_layout.addWidget(self.doubleSpinBox21)
        
        self.l4 = QLabel()
        self.l4.setText("Timeslice Overlap")
        self.main_layout.addWidget(self.l4)
        self.doubleSpinBox22 = QDoubleSpinBox(self.main_widget)
        self.doubleSpinBox22.setProperty("value", 0.5)
        self.doubleSpinBox22.setSingleStep(0.01)
        self.doubleSpinBox22.valueChanged.connect(self.spinState22)        
        self.main_layout.addWidget(self.doubleSpinBox22)


        self.refpressLabel = QLabel("Ref Pressure")
        self.main_layout.addWidget(self.refpressLabel)
        self.refpressComboBox = QComboBox()
        self.refpressComboBox.addItem(RPtext[0])
        self.rpValue = 2.0e-5
        self.refpressComboBox.addItems(["{}".format(RPtext[x]) for x in range(1,2)])
        self.refpress = self.refpressComboBox.currentIndex()
        self.refpressComboBox.currentIndexChanged.connect(self.comboState15)
        self.main_layout.addWidget(self.refpressComboBox) 


    def on_Spl_PlotpushButton_clicked(self):
        print("I reached this point")       
        print("Start the SPL Plot process")
        self.accelcalcs()          # Run accel-time calculations       
        from SPL_Rev5 import QMainWindow
        self.qmw = QMainWindow(self)
        self.qmw.Start_Plot_Spl()
  
    def comboState15(self):
        ci = self.refpressComboBox.currentIndex()
        print("Reference Pressure was changed")
        print('Ref press index : ', ci)
        print(self.RP[ci][0])
        self.rpValue = self.RP[ci][1]
        RPtext_current = self.RPtext[ci]
        self.RPtext_current = RPtext_current
...
# Define Button State for  Spin Boxes and buttons in PSD - Plot
    def spinState20(self):
        print("The npseg value was changed")
        print(self.doubleSpinBox20.value())
        self.dsb1Value = self.doubleSpinBox20.value()

    def spinState21(self):
        print("The Timeslice value was changed")
        print(self.doubleSpinBox21.value())
        self.dsb2Value = self.doubleSpinBox21.value()

    def spinState22(self):
        print("The Timeslice Overlap value was changed")
        print(self.doubleSpinBox22.value())
        self.dsb22Value = self.doubleSpinBox22.value()
